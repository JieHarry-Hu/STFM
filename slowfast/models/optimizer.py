...
def construct_optimizer(model, cfg):
    ...
    optim_params = [
        {"params": bn_parameters, "weight_decay": cfg.BN.WEIGHT_DECAY},  # 0.0
        {"params": non_bn_parameters, "weight_decay": cfg.SOLVER.WEIGHT_DECAY},
        {"params": zero_parameters, "weight_decay": 0.0},
    ]
    optim_params = [x for x in optim_params if len(x["params"])]   # remove empty groups
    ...
    assert len(list(model.parameters())) == len(non_bn_parameters) + len(
        ...
        zero_parameters
    ) + len(no_require_grad), "parameter size does not match: {} + {} + {} + {} != {}".format(
