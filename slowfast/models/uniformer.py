...
import os
from .STFM import STFM, CFBlock, similar_loss
...
class Uniformer(nn.Module):
    def __init__(self, cfg):
        ...
        num_tokens = cfg.STFM.NUM_TOKENS
        dense = cfg.STFM.DENSE
        pos_kernel = cfg.STFM.POS_KERNEL
        similar_loss = cfg.STFM.SIMILAR_LOSS
        ...
        if split:
            ...
            self.blocks3 = nn.ModuleList([
                SABlock(
                    ...
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]], norm_layer=norm_layer)
                for i in range(depth[2])])
            self.blocks40 = nn.ModuleList([
                CFBlock(
                    dim=embed_dim[3],num_heads=num_heads[3], mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, 
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i+depth[0]+depth[1]+depth[2]], act_layer=nn.GELU, norm_layer=nn.LayerNorm)
            for i in range(depth[3])])
        
        self.stfm = STFM(img_size=img_size, patch_size=16, dim=embed_dim[3], num_tokens=num_tokens, input_channel=embed_dim[2],
                        dense=dense, pos_kernel=pos_kernel, add_loss=similar_loss, T_multiple=2)
        self.normF = nn.LayerNorm(embed_dim[-1])
    ...
    def get_pretrained_model(self, cfg):
        if cfg.UNIFORMER.PRETRAIN_NAME:
            ...
            pre_state_dict = {k:v for k,v in checkpoint.items() if k in state_dict_3d and v.size() == state_dict_3d[k].size()}
            self.pre_state_dict = pre_state_dict
            state_dict_3d.update(pre_state_dict)
            return state_dict_3d, pre_state_dict
        else:
            return None, None
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks1):
            if self.use_checkpoint and i < self.checkpoint_num[0]:  #Flase
        ...
        for i, blk in enumerate(self.blocks3):
            if self.use_checkpoint and i < self.checkpoint_num[2]:
                ...
                x = blk(x)
        x, pos = self.stfm(x, ori_img)
        del ori_img
        if similar_loss:
            s_loss = self.stfm.sloss(x)
        else:
            s_loss = torch.tensor(0.0)
        for i, blk in enumerate(self.blocks40):
        ...
        x = self.normF(x)
        x = self.pre_logits(x)
        return x, s_loss  #[B T N C], loss
    def forward(self, x):
        x = x[0]
        x, s_loss = self.forward_features(x)
        x = x.permute(0, 3, 1, 2).flatten(2).mean(-1)
        x = self.head(x)
        return x#, s_loss
