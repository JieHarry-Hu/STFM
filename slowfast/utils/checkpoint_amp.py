...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if "model_state" in checkpoint.keys():
            model_state = checkpoint["model_state"]  #checkpoint的参数键值对
        else:
            model_state = checkpoint
        model_state = normal_to_sub_bn(
            model_state, model_state_dict_3d
        )  #返回的是checkpoint的参数键值对
        if inflation:
            ...
            ms.load_state_dict(pre_train_dict_match, strict=False)#strict=False表示,有什么加载什么，不要求完全一一对应
            epoch = -1
            for p,v in ms.named_parameters():
                print(p,'requires_grads:',v.requires_grad)
