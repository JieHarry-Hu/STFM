...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            # Compute the loss.
            loss = loss_fun(preds, labels)# + similar_loss
        ...
        loss_scaler(loss, optimizer, iter=cur_iter, clip_grad=cfg.SOLVER.CLIP_GRADIENT, parameters=model.parameters(), create_graph=is_second_order)
        # Perform the backward pass.
        if (cur_iter+1) % 4 == 0:
            optimizer.zero_grad()
        #print('tansconv:',model.state_dict()['stfm.adapt.transconv.weight'])
        if cfg.MIXUP.ENABLE:
...
def train(cfg):
    ...
    if du.is_master_proc() and cfg.LOG_MODEL_INFO:
        misc.log_model_info(model, cfg, use_train_input=True)
    #freeze
    if cfg.MODEL.FINETUNNING:
        for n, p in model.named_parameters():
            if 'stfm' in n or 'normF' in n or 'blocks40' in n or 'head' in n:
                print('grad: ', n)
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        if cfg.MULTIGRID.LONG_CYCLE:  # False by default
